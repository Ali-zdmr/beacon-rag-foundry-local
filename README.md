...
- **The UI is intentionally minimal** - one chat box, one status line
  (documents indexed, active backend). The goal was a functioning grounded
  Q&A demo per the assignment brief, not a themed dashboard.
...
src/
  ...
  webapp.py      Flask app serving the chat UI + JSON API
